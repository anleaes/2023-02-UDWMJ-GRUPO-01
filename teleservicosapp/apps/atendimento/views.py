...
from .models import Atendimento , MateriaisAtendimento, Materiais, Client
...
def list_atendimento(request):
    ...
    materiaisatendimento = MateriaisAtendimento.objects.filter()
...
def add_materiais_atendimento(request, id_atendimento):
    ...
    context = {}
    if request.method == 'POST':
        form = MateriaisAtendimento(request.POST)
        if form.is_valid():
            f = form.save(commit=False)
            f.atendimento = Atendimento.objects.get(id=id_atendimento)
            f.save()
            form.save_m2m()
            return redirect('atendimentos:list_atendimentos')
...
def delete_materiais_atendimento(request, id_materiais_atendimento):
    materiaisatendimento = MateriaisAtendimento.objects.get(id=id_materiais_atendimento)
